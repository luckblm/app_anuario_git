...
demografia_pa_Server <- function(id) {
  moduleServer(id, function(input, output, session) {
    ...
    fonte <- reactive({
      demo1f <- demo1f %>% filter(ano == input$demo1ano)
      if (demo1f$nota1 == "-" && demo1f$nota2 == "-") {
        tagList(
          tags$h6(tags$b("Fonte: "), demo1f$fonte, style = 'font-family: sans-serif;'),
          tags$h6(tags$b("Elaboração: "), demo1f$elaboracao)
        )
      } else if (demo1f$nota1 != "-" && demo1f$nota2 == "-") {
        tagList(
          tags$h6(tags$b("Fonte: "), demo1f$fonte, style = 'font-family: sans-serif;'),
          tags$h6(tags$b("Elaboração: "), demo1f$elaboracao),
          tags$h6(tags$b("Nota: "), demo1f$nota1)
        )
      } else if (demo1f$nota1 != "-" && demo1f$nota2 != "-") {
        tagList(
          tags$h6(tags$b("Fonte: "), demo1f$fonte, style = 'font-family: sans-serif;'),
          tags$h6(tags$b("Elaboração: "), demo1f$elaboracao),
          tags$h6(tags$b("Nota¹: "), demo1f$nota1),
          tags$h6(tags$b("Nota²: "), demo1f$nota2)
        )
      }
    })
    
    output$rodape1 <- renderUI({
      fonte()
    })
    output$rodape2 <- renderUI({
      fonte()
    })
    output$rodape3 <- renderUI({
      fonte()
    })
